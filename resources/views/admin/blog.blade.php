                        ...
                        <table class="table table-striped" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th width="15%">{{__('admin.Created At')}}</th>
                                    <th width="15%">{{__('admin.Updated At')}}</th>
                                    <th width="15%">{{__('admin.Action')}}</th>
                                  </tr>
                            </thead>
                            <tbody class="sortable-posts list-unstyled">
                                @foreach ($blogs as $index => $blog)
                                    <tr id="{{ $blog->id }}" data-id="{{ $blog->id }}" data-ordering="{{$blog->ordering}}">
                                        <td>{{ ++$index }}</td>
...
<script>
    ...
    $(".sortable-posts").sortable({
              stop: function (event, ui) {
                  var orderData = [];
                  $(".sortable-posts").find('tr').each(function () {
                      orderData.push({
                          id: $(this).data('id'),
                          order: $(this).data('ordering')
                      });
                  });
                console.log(orderData);
                  $.ajax({
                      ...
                      method: 'POST',
                      data: { orderData: orderData,_token : '{{ csrf_token() }}' },
